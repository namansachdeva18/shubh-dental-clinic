...
import { ChevronDown, HelpCircle, Sparkles, Stethoscope, ShieldCheck, MapPin } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { accordionContent } from '../lib/motion';
import ScrollReveal from './ScrollReveal';

const FAQ_CATEGORIES = [
  { id: 'all', label: 'All FAQs (15)', icon: Sparkles },
  { id: 'doctor', label: 'Doctor & Top', icon: Stethoscope },
  { id: 'aligners', label: 'Aligners & Braces', icon: Sparkles },
  { id: 'implants', label: 'Implants & Care', icon: ShieldCheck },
  { id: 'clinic', label: 'Clinic & Visits', icon: MapPin },
];
...
export default function FAQSection() {
  ...
  // If expandedAll is false, show only first 5 FAQs
  const displayedFaqs = expandedAll ? faqs : faqs.slice(0, 5);
    ...
    <section id="faq" className="faq-section-wrapper">
      <div className="faq-container">
        ...
        <ScrollReveal className="faq-header text-center">
          ...
          <h2 className="faq-title font-heading">
            Frequently Asked <span className="gold-gradient-text">Questions</span>
          </h2>
          <p className="faq-subtitle">
            Find instant answers regarding orthodontic aligners, braces, implants, and visiting centres.
          </p>
        </ScrollReveal>

        {/* Compact Accordion List (Top 5 + Expandable) */}
        ...
        {/* Compact Dropdown / Toggle for Remaining FAQs */}
        <div className="faq-more-dropdown-wrap">
          <button
            type="button"
          ...
          >
            <span className="faq-toggle-dot" />
            <span>{expandedAll ? 'Show Less (Top 5 FAQs)' : `View All ${faqs.length} FAQs (10 More)`}</span>
            <ChevronDown 
              size={14} 
              style={{ 
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .faq-more-dropdown-wrap {
          display: flex;
          justify-content: center;
          margin-top: 1rem;
        }

        .faq-dropdown-toggle-btn {
          display: inline-flex;
          align-items: center;
          gap: 0.5rem;
          background: #FAF8F5;
        ...
        .faq-item {
          ...
          transition: all 0.22s ease;
          overflow: hidden;
          box-shadow: 0 2px 8px rgba(74, 37, 24, 0.03);
        }

        .faq-item:hover {
          border-color: rgba(214, 122, 65, 0.35);
          box-shadow: 0 4px 14px rgba(74, 37, 24, 0.06);
        }

        .faq-item-open {
          border-color: rgba(214, 122, 65, 0.5);
          background: #FFFDFB;
          box-shadow: 0 6px 20px rgba(74, 37, 24, 0.06);
        }
        ...
        @media (max-width: 768px) {
          .faq-section-wrapper { padding: 1.25rem 0.75rem !important; }
          .faq-header { margin-bottom: 0.85rem !important; }
          .faq-badge {
